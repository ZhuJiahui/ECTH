...
def to_vsm(read_directory1, read_directory2, write_directory):
    '''
    微博文本的向量空间构造，值为TF
    ...
    for i in range(file_number):
        ...
        while line:
            ...
            for j in range(len(this_row)):
                try:
                    tf_dict[this_row[j].split(',')[0]] += 1
                except KeyError:
                    tf_dict[this_row[j].split(',')[0]] = 0
...
if __name__ == "__main__":
    ...
    write_directory = root_directory + u'dataset2/text_model/vsm'
    
    if (not(os.path.exists(write_directory))):
        os.mkdir(write_directory)
    
    to_vsm(read_directory1, read_directory2, write_directory)
